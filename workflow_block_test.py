    ...
    for i in range(len(chosn_index)):
        ...
        if index1 != len(base)-1:
            ...
            if index2!=0:
                ...
                finer_z[chosn_index[i]]=2 * base_z[index1]*index2/deci_ratio + chosn_r[i]
            else:
                finer[chosn_index[i]] = base[index1]
                finer_r[chosn_index[i]] = base_r[index1]
                finer_z[chosn_index[i]] = base_z[index1]
...
def partial_refine(deci_ratio,timestep,psnr,ctag,update_interval,thre):
    ...
    thre, sig = get_prediction_threshold(dc, s_amp, s_freq, s_phase, timestep)
    ...
    #aa=time.time() 
    #finer, finer_r,finer_z = partial_refinement_new(delta_index, finer_len, delta_L0_L1, delta_r_L0_L1, delta_z_L0_L1, dpot_L1, r_L1, z_L1, deci_ratio)
    #bb =time.time()
...
#partial_refine(deci_ratio,25,"false",1,5,40)
#try:
#	cluster = rados.Rados(conffile='')
#except TypeError as e:
#    print 'Argument validation error: ', e
...
#ioctx_2 = cluster.open_ioctx('tier2_pool')
#start=time.time()
#delta_L0_L1_str = ioctx_2.read("delta_L0_L1_0.99",ioctx_2.stat("delta_L0_L1_0.99")[0],0)
#end = time.time()
#delta_read_time = end-start
#print "Delta reading time = ", delta_read_time
#ioctx_2.close()
