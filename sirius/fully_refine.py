...
#print "single refinement time whole=",s_refine_t_w
for i in range(len(time_single)-1):
    prefix_time += str(time_single[i])+","
prefix_time += str(time_single[-1])
...
x=np.arange(1/sample_rate,1/sample_rate+1*Nsamples/sample_rate,1/sample_rate)
...
time_p, noise_index = preprocess_noise(time_single)

noise_sum=0.0
noise_num=0
noise_len=[]
for i in range(len(noise_index)):
    noise_len.append(noise_index[i][1]-noise_index[i][0]+1)
    for j in range(noise_index[i][0],noise_index[i][1]+1):
        #print y[j]
        noise_sum+=time_p[j]
        noise_num+=1
noise = noise_sum/noise_num
print "noise=",noise
print "noise time=", (np.mean(noise_len)-1)*1/sample_rate    

time_p -= np.mean(time_p)
freq, FFT_y=FFT(sample_rate, time_p)
org_mean = np.mean(FFT_y)
T = []
for i in range(len(FFT_y)):
    if FFT_y[i]>20 * org_mean:
        print i,FFT_y[i]
        T.append(1/freq[i])

print T
threshold = noise_threshold(noise)

deci_ratio=2
chosn_index_L1 = find_augment_points(dpot_L1,[range(len(dpot_L1))],deci_ratio, 1-threshold)
chosn_index_group_L0=[]
for i in chosn_index_L1:
    chosn_index_group_L0.append(i[0] * deci_ratio)
    chosn_index_group_L0.append(i[-1] * deci_ratio)
chosn_data_L0=[]
chosn_r_L0=[]
chosn_z_L0=[]
for i in chosn_index_L1:
