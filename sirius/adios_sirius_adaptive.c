...
///////////
        ...
        else
        {
		    ...
		    */
		    ...
		    rados_ioctx_t io_ssd;
                    rados_ioctx_t io_hdd;
		    char * hdd_poolname = "tier2_pool";
		    char * ssd_poolname = "tier0_pool";

		    err = rados_ioctx_create(cluster, ssd_poolname, &io_ssd);
		    if (err < 0) {
	                    fprintf(stderr, "Cannot open rados pool %s: %s\n", ssd_poolname, strerror(-err));
...
#ifdef WHOLE_BLOCK
                    char sname_buf[64]={"Temperature_"};
		    char dname_buf[64]={};
		    sprintf(dname_buf,"%f", data_reduced[0]);
		    strcat(sname_buf, dname_buf);
                    err = rados_write(io_ssd, "dpot_L1", data_reduced, sizeof(double) * nvertices_new, 0);
		    
                    if (err < 0) {
                                fprintf(stderr, "Cannot write object \"dpot_L1\" to pool %s: %s\n",  ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                exit(1);
                    } else {
                                printf("\nWrote Reduced L1 data to object \"dpot_L1\".\n");
                    }
		    
		    err = rados_write(io_ssd, "r_L1", r_reduced, sizeof(double) * nvertices_new, 0);

                    if (err < 0) {
                                fprintf(stderr, "Cannot write object \"r_L1\" to pool %s: %s\n",  ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                exit(1);
                    } else {
                                printf("\nWrote Reduced L1 data to object \"r_L1\".\n");
                    }

		    err = rados_write(io_ssd, "z_L1", z_reduced, sizeof(double) * nvertices_new, 0);

                    if (err < 0) {
                                fprintf(stderr, "Cannot write object \"z_L1\" to pool %s: %s\n",  ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                exit(1);
                    } else {
                                printf("\nWrote Reduced L1 data to object \"z_L1\".\n");
                    }


			err = rados_write(io_ssd, "data", data, sizeof(double) * nelems, 0);
                    ...
                    if (err < 0) {
                                ...
                                printf("\nWrote Reduced L1 data to object \"data\".\n");
                    }
		    ...
		    char sname_buf[64]={"Temperature_"};
		    char sname_backup[64]={"Temperature_"};
		    char dname_buf[64]={};
                    double * blockdata = 0;
                    blockdata = (double *) malloc (blocksize);
                    int numblock = 0;
                    int remain = 0;
                    int blocklen = 0;
                    int blockbytes = 0;
                    numblock = nvertices_new * 8 / blocksize;
		    char name[numblock+1][64]; 
                    if (nvertices_new * 8 <= blocksize)
                    {
                        printf("block less then 1 MB\n");
                        sprintf(dname_buf,"%f", data_reduced[0]);
                        strcat(sname_buf, dname_buf);
			err = rados_write(io_ssd, sname_buf, data_reduced, sizeof(data_reduced)* nvertices_new, 0);
                     	if (err < 0) {
                                fprintf(stderr, "Cannot write object \"%s\" to pool %s: %s\n", sname_buf,  ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                exit(1);
                        } else {
                                printf("\nWrote Reduced L1 data to object \"%s\".\n", sname_buf);
                        }
                    }
                    else if (nvertices_new * 8 > blocksize) 
                    {
			
			printf("Block larger than 1 MB\n");
                        printf("Then number of 1 MB blocks = %d\n", numblock);
                        remain = nvertices_new * 8 % blocksize;
                        for(int i = 0; i < (numblock + 1); i++)
			{
				
                                memset(sname_buf, 0, sizeof(char) * 64);
                                memset(blockdata, 0, blocksize);                               
                                if (i == numblock) { 
                                	blocklen = remain/ sizeof(double);
					blockbytes = remain;
				}
				else { 
					blocklen = blocksize / sizeof(double);
                                        blockbytes = blocksize;
				}
				memcpy(blockdata, data_reduced, blocklen);
                                memcpy(sname_buf, sname_backup, 64);
				sprintf(dname_buf,"%f",blockdata[0]);
				strcat(sname_buf, dname_buf);
                                strcpy(name[i], sname_buf);
                                err = rados_write(io_ssd, sname_buf, blockdata, blockbytes, 0);
                                if (err < 0) {
                                	fprintf(stderr, "Cannot write object \"%s\" to pool %s: %s\n", sname_buf,  ssd_poolname, strerror(-err));
                                	rados_ioctx_destroy(io_ssd);
                                	rados_shutdown(cluster);
                                	...
                                	printf("\nWrote Reduced L1 data block %d to object \"%s\".\n", i, sname_buf);
                               		}
                        	if (i < numblock) data_reduced = data_reduced + blocksize / sizeof (double);
			}
                        data_reduced = data_reduced - numblock * blocksize / sizeof(double);
                    }
		    free(blockdata);
                    ...
                    //free (data_reduced);
	            //free (blockdata);	
		    char delta_sname_buf[128] = {"Delta_"};
                    if (save_delta)
                    {	
			err = rados_ioctx_create(cluster, hdd_poolname, &io_hdd);
                        ...
                        char delta_dname_buf[128]={};
#ifdef SMALL_BLOCK
			char delta_sname_backup[128]={"Delta_"};
			int delta_numblock = 0;
			delta_numblock = nelems * 8 / blocksize;
			char name_delta[delta_numblock+1][128];
			double * delta_blockdata = 0;
                        delta_blockdata = (double *) malloc (blocksize);
                        int delta_remain = 0;
                        int delta_blocklen = 0;
                        int delta_blockbytes = 0;
#endif
                        if (!compress_delta)
                        {
#ifdef WHOLE_BLOCK
                            //char delta_sname_buf[64] = {"Delta_"};
                            sprintf(delta_dname_buf,"%f", delta[0]);
                            strcat(delta_sname_buf, delta_dname_buf);
...
#ifdef SMALL_BLOCK 
                            if (nelems * 8 <= blocksize)
			    {
				printf("Delta block less then 1 MB\n");
				sprintf(delta_dname_buf,"%f", delta[0]);
                           	strcat(delta_sname_buf, delta_dname_buf);

                            	err = rados_write(io_hdd, delta_sname_buf, delta, sizeof(double) * nelems, 0);
                            	if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"%s\" to pool %s: %s\n", delta_sname_buf,  hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                                        exit(1);
                            	} else {
                                        printf("\nWrote Delta to object \"%s\".\n", delta_sname_buf);
                            	}
			    }
			    else if (nelems * 8 > blocksize)
			    {
				printf("Delta block larger than 1 MB\n");
				printf("Then number of 1 MB delta blocks = %d\n", delta_numblock);
				delta_remain = nelems * 8 % blocksize;

				for (int i = 0; i < (delta_numblock + 1); i++)
				{
					printf("8\n");
					memset(delta_sname_buf, 0, sizeof(char) * 128);
					printf("6\n");
                                	memset(delta_blockdata, 0, blocksize);      
					printf("7\n");                   
					if (i == delta_numblock) {
                                        	delta_blocklen = delta_remain/ sizeof(double);
                                        	delta_blockbytes = delta_remain;
                                	}
                               		else {
                                        	delta_blocklen = blocksize / sizeof(double);
                                        	delta_blockbytes = blocksize;
                                	}
					printf("delta_remain = %d\n",delta_remain);
					memcpy(delta_blockdata, delta, delta_blocklen);
					printf("1\n");
					memcpy(delta_sname_buf, delta_sname_backup, 128);
					printf("2\n");
					for (int j =0;j < 100;j++)
						printf("delta_blockdata[%d] = %f\n", j,delta_blockdata[j]);                                  
					sprintf(delta_dname_buf,"%f",delta_blockdata[0]);
					printf("3\n");
					strcat(delta_sname_buf, delta_dname_buf);
					printf("4\n");
					strcpy(name_delta[i],delta_sname_buf);
					printf("5\n");
					err = rados_write(io_hdd, delta_sname_buf, delta_blockdata, delta_blockbytes, 0);
					if (err < 0) {
                                        	fprintf(stderr, "Cannot write object \"%s\" to pool %s: %s\n", delta_sname_buf,  ssd_poolname, strerror(-err));
                                        	rados_ioctx_destroy(io_hdd);
                                        	rados_shutdown(cluster);
                                        	exit(1);
					} else {
                                        	printf("\nWrote Delta data block %d to object \"%s\".\n", i, delta_sname_buf);
                                        }
					if (i < delta_numblock) delta = delta + blocksize / sizeof(double);
				}
				delta = delta - delta_numblock * blocksize / sizeof(double);
			    }
#endif			
			    ...
			    double ceph_end_time = MPI_Wtime ();
                            ...
                            //printf("Write delta successfully!\n");
#endif                           
                        }
                        else
                        {
			...
			rados_completion_t comp;
                        ...
                        double ceph_start_read = 0.0;
			double ceph_end_read = 0.0;
			
			ceph_start_read = MPI_Wtime ();
			double ratio = 0.5;
#ifdef WHOLE_BLOCK
                        double * read_res_data = 0;
			double * read_res_delta = 0;
                        read_res_data = (double *) malloc (nvertices_new * 8 * ratio);
			read_res_delta = (double *) malloc (nelems * 8 * ratio);

                        err = rados_aio_read(io_ssd, sname_buf, comp, read_res_data, sizeof(data_reduced)* nvertices_new * ratio, 0);
                        if (err < 0) {
                                fprintf(stderr, "Cannot read object. %s %s\n", ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                exit(1);
                        } else {
                                printf("\nRead data object \"%s\"", sname_buf);
                        }

                        // Wait for the operation to complete 
                        rados_aio_wait_for_complete(comp);
                        //printf("The data contents are:\n %f \n", read_res_data[0]);
			err = rados_aio_read(io_hdd, delta_sname_buf, comp, read_res_delta, sizeof(delta)* nelems * ratio, 0);
                        if (err < 0) {
                                fprintf(stderr, "Cannot read object. %s %s\n", ssd_poolname, strerror(-err));
                                rados_ioctx_destroy(io_ssd);
                                rados_shutdown(cluster);
                                ...
                                printf("\nRead delta object \"%s\"", delta_sname_buf);
                        }

                        // Wait for the operation to complete 
                        rados_aio_wait_for_complete(comp);
                        //printf("The delta contents are:\n %f \n", read_res_delta[0]);
			//free(read_res_data);
			//free(read_res_delta);
#endif                        
...
#ifdef SMALL_BLOCK
			double * read_res = 0;
                        read_res = (double *) malloc (blocksize);
                        char read_name_buf[64] = {};
			int num_read = 0;
			int read_size = nvertices_new * ratio * sizeof(double); 
			if (read_size % blocksize != 0)
				num_read = nvertices_new * ratio * 8 /blocksize +1;
			else 
				num_read = nvertices_new * ratio * 8 /blocksize;

                        for (int i = 0; i < num_read; i++)
			{
                        	err = rados_aio_read(io_ssd, name[i], comp, read_res, blocksize, 0);
                        	if (err < 0) {
                                	fprintf(stderr, "Cannot read object. %s %s\n", ssd_poolname, strerror(-err));
                                	rados_ioctx_destroy(io_ssd);
                                	rados_shutdown(cluster);
                                	...
                                	printf("\nRead data block %d object \"%s\"", i, name[i]);
                        	}
				rados_aio_wait_for_complete(comp);
			}
                        
                        int num_read_delta = 0;
                        int read_delta_size = nelems * ratio * sizeof(double); 
			if (read_delta_size % blocksize != 0)
                                num_read_delta = nelems * ratio * 8 /blocksize +1;
                        else
                                num_read_delta = nelems * ratio * 8 /blocksize;

			for (int i = 0; i < num_read_delta; i++)
                        {
                                err = rados_aio_read(io_hdd, name_delta[i], comp, read_res, blocksize, 0);
                                if (err < 0) {
                                        fprintf(stderr, "Cannot read object. %s %s\n", ssd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_ssd);
                                        rados_shutdown(cluster);
                                        ...
                                        printf("\nRead delta No.%d object \"%s\"", i ,name_delta[i]);
                                }
                                rados_aio_wait_for_complete(comp);
                        }
			//free(read_res);
			...
			ceph_end_read = MPI_Wtime ();
			printf("Read time = %f\n", ceph_end_read - ceph_start_read);
			*/
