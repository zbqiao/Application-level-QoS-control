        ...
        if i == len(chosn_index)-2:
            for j in range(start,(len(base)-1)*deci_ratio+1):
                ...
                if index1!=len(base)-1:
                    ...
                    #finer_z_p.append(base_z[index1])
        i+=2
    print "percentage of selected points= ", num_refine/(82474-len(base))
    #for i in chosn_index:
    #    if i[-1] != len(base)-1:
...
for i in range(1):
    ...
    #lenlen=0
    #for i in range(0,len(chosn_index_L1),2):
    #    lenlen+=chosn_index_L1[i+1]-chosn_index_L1[i]+1
    #print lenlen
    #print "read_chosn_time",time.time()-chosn_b
    ...
    #whole_str = ioctx_2.read("delta_L0_L1",ioctx_2.stat("delta_L0_L1")[0],0)
    #tag='d'
    #print int(ioctx_2.stat("delta_L0_L1")[0]/8)-1
    #for i in range(int(ioctx_2.stat("delta_L0_L1")[0]/8)-1):
    #	tag+='d'
    #whole = struct.unpack(str(int(ioctx_2.stat("delta_L0_L1")[0]/8))+'d',whole_str)
    #w2=time.time()
    #print "wtime=",w2-w1
    #p1=time.time()
    #part_str = ioctx_2.read("delta_L0_L1",24,0)
    #a=str(3)
    #part = struct.unpack('3d',part_str)
    #p2=time.time()
    #print susu_s:
    #print "ltime",p2-p1
    #print ioctx_2.stat("chosn_data_L1")[0]
    #before = time.time()
    chosn_data_L0_str = ioctx_2.read("chosn_data_L0",ioctx_2.stat("chosn_data_L0")[0],0)
    #after = time.time()
    #s_read_t=after-before
    ...
    #b=time.time()
    #print "read and decode=",b-a

    #chosn_index_L1=[]
    #for i in range(0,len(chosn_index_group_L1),2):
    #    chosn_index_L1.append(range(chosn_index_group_L1[i],chosn_index_group_L1[i+1]+1))    
...
#    s_start=0
#    for i in range(0,len(chosn_index_group_L1),2):
#        s_len = chosn_index_group_L1[i+1]-chosn_index_group_L1[i]+1
#        temp = chosn_data_1d[s_start:s_start+s_len]
#        temp_r = chosn_r_1d[s_start:s_start+s_len]
#        temp_z = chosn_z_1d[s_start:s_start+s_len]
#        chosn_data.append(temp)
#        chosn_r.append(temp_r)
#        chosn_z.append(temp_z)
#        s_start=s_len
#        temp=[]
#    	temp_r=[]
#    	temp_z=[]
 
#    for i in range(len(chosn_index_L0)):
#        temp_str = ioctx_2.read("delta_L0_L1",(chosn_index_L0[i][-1]*deci_ratio-chosn_index_L0[i][0]*deci_ratio+1)*8,chosn_index_L0[i][0]*deci_ratio*8)
#        temp_r_str = ioctx_2.read("delta_r_L0_L1",(chosn_index_L0[i][-1]*deci_ratio-chosn_index_L0[i][0]*deci_ratio+1)*8,chosn_index_L0[i][0]*deci_ratio*8)
#        temp_z_str = ioctx_2.read("delta_z_L0_L1",(chosn_index_L0[i][-1]*deci_ratio-chosn_index_L0[i][0]*deci_ratio+1)*8,chosn_index_L0[i][0]*deci_ratio*8)
#        tag=str(chosn_index_L0[i][-1]*deci_ratio-chosn_index_L0[i][0]*deci_ratio+1)
#		
#        temp=struct.unpack(tag+'d',temp_str)
#        temp_r=struct.unpack(tag+'d',temp_r_str)
#        temp_z=struct.unpack(tag+'d',temp_z_str)
#        chosn_data.append(temp)
#        chosn_r.append(temp_r)
#        chosn_z.append(temp_z)
#        temp=[]
...
#        temp_z_str=[]
    ...
    #c = time.time()
    #print "Time for 1d to 2d=",c-b
    finer_L0, finer_r_L0,finer_z_L0 = refinement(chosn_index_L0,chosn_data_L0,chosn_r_L0,chosn_z_L0,dpot_L1,r_L1,z_L1,deci_ratio)
    #d = time.time()
    #s_read_t=d-b
