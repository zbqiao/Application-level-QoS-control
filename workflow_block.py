...
def fully_refine(deci_ratio,timestep,tag,update_interval,thre):
    ...
    #bb=time.time()
    ##finer, finer_r,finer_z = partial_refinement_new(range(len(delta_L0_L1)), len(delta_L0_L1), delta_L0_L1, delta_r_L0_L1, delta_z_L0_L1, dpot_L1, r_L1, z_L1, deci_ratio)
...
def partial_refine(deci_ratio,timestep,psnr,ctag,update_interval,thre, peak_noise, no_noise, time_interval):
#def partial_refine(deci_ratio,timestep,psnr,ctag,update_interval,thre, peak_noise, no_noise, time_interval,original_data, original_r,original_z,delta,delta_r,delta_z):
    ...
    #if 1==0:
    if sig > peak_noise:
        #finer, finer_r,finer_z, finer_p, finer_r_p,finer_z_p = partial_refinement_new(delta_index, finer_len, delta_L0_L1, delta_r_L0_L1, delta_z_L0_L1, dpot_L1, r_L1, z_L1, deci_ratio,original_data,original_r,original_z,delta,delta_r,delta_z)
        ...
        finer_r_p = r_L1
        finer_z_p = z_L1        
    finer_name = "xgc/60_130_nyquist_xgc/60_130_35/xgc_"+str(peak_noise)+"_"+str(no_noise)+"_"+str(timestep)+"_"+str(ctag)+".npz"
...
def work_flow(deci_ratio, time_interval, psnr, update_interval,high_potential_threshold,peak_noise, no_noise):
    tag = 0
    timestep = 0
    last_tag = 0
    for i in range(0,3000,time_interval):
        ...
        if int(i/time_interval) %(update_interval-1) == 1 and i !=time_interval:
            ...
            t = np.arange(time_interval,time_interval*update_interval, time_interval)   
            sig = dc  
        ...
        if tag == 0:
            ...
            print i, update_interval, time_interval
            ...
            print timestep
            partial_refine(deci_ratio, timestep, psnr, tag,update_interval,high_potential_threshold, peak_noise, no_noise, time_interval)         
        end = time.time()
